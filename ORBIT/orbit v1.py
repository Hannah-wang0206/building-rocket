...
def plot_orbit(target="Moon", angle=45, velocity=8000):
    try:
        # Get target data
        target_data = celestial_bodies[target]
        r_target = target_data["distance"]

        # Low Earth Orbit (LEO) radius (700 km above Earth's surface)
        r_low_orbit = 7000000  # Low Earth orbit radius in meters (700 km above surface)
        ...
        a_transfer = a_transfer * (velocity_orbit / velocity)  # Modify semi-major axis
...
# User input for velocity (in m/s) and angle (in degrees)
...
plot_orbit(target=target, angle=angle, velocity=velocity)
